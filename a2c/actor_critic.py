...
class ActorCritic(nn.Module):
    ...
    def _forward_rnn(self, x, hiddens, masks):
        ...
        n_steps = int(x.size(0) / hiddens.size(0))
        n_processes = hiddens.size(0)
        x = x.view(n_steps,n_processes,-1) #(step, n_processes, -1) 
        masks = masks.view(n_steps, n_processes, -1) #(step, n_processes, -1) 
        #outputs, _ = self.rnn(x)
        
        outputs = []
        hiddens_list = [hiddens.unsqueeze(0)] #(1, n_processes, hidden_size)
        for step in range(n_steps): 
            hidden = masks[step].unsqueeze(0) * hiddens_list[step]
            output, hidden = self.rnn(x[step].unsqueeze(0), hidden)
            outputs.append(output.squeeze(0))
            hiddens_list.append(hidden)
        hiddens_list = torch.stack(hiddens_list).squeeze(1) 
        
        #outputs = torch.stack(outputs).squeeze(0)
        outputs = outputs.view(n_steps * n_processes, -1)   
    ...
    def get_estimate_returns(self, inputs, hiddens, masks):
        x = self.head(inputs / 255.0)
        if self.recurrent:
            x, hiddens = self._forward_rnn(x, hiddens, masks)
        values = self.critic(x)
    ...
    def get_action_probs(self, inputs, hiddens, masks):
        x = self.head(inputs / 255.0)
        if self.recurrent:
            x, hiddens = self._forward_rnn(x, hiddens, masks)
        action_probs = self.actor(x)
        action_probs = F.softmax(action_probs, -1)
        return action_probs, hiddens
